...
export default function Login() {
  ...
  // Capture referral code from ?ref= and persist for Google sign-up flow
  useEffect(() => {
    const ref = searchParams.get('ref');
    if (ref) {
      localStorage.setItem('bibleBio_referral', ref);
    }
  }, [searchParams]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg)] px-6">
      ...
      >
        ...
        <div className="flex items-center gap-3 my-6">
          ...
          <span className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)]">or</span>
          <div className="flex-1 h-px" style={{ background: 'var(--border)' }}></div>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="w-full py-3.5 rounded-full font-semibold flex items-center justify-center gap-3 glass hover:bg-white/5 transition-colors"
